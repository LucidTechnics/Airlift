...
public class JavaScriptingUtil
{
	...
	private void executeHandler(String _scriptResource, Context _context)
	{
		try
		{
			...
			String requireHandler = "var handle = function(_WEB_CONTEXT) { " +
									"var web = require('airlift/web').create(_WEB_CONTEXT); " +
									"try { require(\"" + scriptResource.replaceAll(".js$", "") + "\").handle(web); } " +
									"catch(e) { if (e.r && e.h) { web.setResponseCode(e.r); web.setContent(JSON.stringify(e.error)); " +
									"e.rhinoException && e.rhinoException.printStackTrace(); } " +
									"else if (e.rhinoException) { e.rhinoException.printStackTrace(); throw e; } " +
									"else if (e.javaException) { e.javaException.printStackTrace(); throw e; } " +
									"else { throw e; } }}";
